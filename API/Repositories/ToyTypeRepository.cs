...
namespace API.Repositories
{
    public class ToyTypeRepository : IToyTypeRepository
    {
        ...
        public async Task<ToyType> AddToyTypeAsync(ToyTypeDto toyType)
        {
            ...
            var newToyType = toyType.ToModel();
        ...
        public async Task<ToyType> DeleteToyTypeAsync(int id)
        {
            ...
            var toyType = await _context.ToyTypes.FirstOrDefaultAsync(t => t.Id == id && t.Deleted == false) ?? throw new Exception($"Toy type with ID {id} not found.");
